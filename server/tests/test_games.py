"""
Unit Tests for Games API Routes - Tailspin Toys Crowd Funding Platform

This module contains comprehensive unit tests for the games API endpoints.
Tests cover successful data retrieval, error handling, and data structure
validation using an in-memory SQLite database.
"""
...
class TestGamesRoutes(unittest.TestCase):
    ...
    Tests the games REST API endpoints including listing all games
    and retrieving individual games by ID. Uses an in-memory database
    with seeded test data for consistent, isolated testing.
    """
    ...
    def setUp(self) -> None:
        ...
        Creates a fresh Flask app with in-memory SQLite database,
        registers the games blueprint, and seeds with test data.
        """
    ...
    def tearDown(self) -> None:
        ...
        Removes database session, drops all tables, and disposes
        of the database engine to prevent connection leaks.
        """
        with self.app.app_context():
            db.session.remove()
            db.drop_all()
    ...
    def _seed_test_data(self) -> None:
        """
        Helper method to seed the test database with sample data.
        
        Creates test publishers, categories, and games based on the
        TEST_DATA configuration for consistent test scenarios.
        """
    ...
    def test_get_games_success(self) -> None:
        ...
        Verifies that the games list endpoint returns all games
        with correct data structure and content matching the
        seeded test data.
        """
        # Act
    ...
    def test_get_games_structure(self) -> None:
        ...
        Validates that the response is a list containing the expected
        number of games and that each game has all required fields.
        """
        # Act
        response = self.client.get(self.GAMES_API_PATH)
        data = self._get_response_data(response)
    ...
    def test_get_game_by_id_success(self) -> None:
        ...
        Verifies that the single game endpoint returns the correct
        game data when provided with a valid game ID.
        """
    ...
    def test_get_game_by_id_not_found(self) -> None:
        ...
        Verifies that the API returns a 404 error with appropriate
        error message when requesting a game that doesn't exist.
        """
        # Act
